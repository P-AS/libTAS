...
size_t SaveStateSaving::queuePageSave(char* addr)
{
    ...
    if (Global::shared_config.savestate_settings & SharedConfig::SS_COMPRESSED) {
        ...
        if (Global::shared_config.savestate_settings & SharedConfig::SS_INCREMENTAL) {
            ...
            compressed_size = LZ4_compress_fast(addr, queued_compressed_base_addr + queued_compressed_size + sizeof(int), static_cast<int>(page_size), queued_compressed_max_size - (queued_compressed_size + sizeof(int)), 1);
        }
        else {
            compressed_size = LZ4_compress_fast_continue(&lz4s, addr, queued_compressed_base_addr + queued_compressed_size + sizeof(int), static_cast<int>(page_size), queued_compressed_max_size - (queued_compressed_size + sizeof(int)), 1);
        }
        if (compressed_size) {
            ...
            if ((queued_compressed_max_size - queued_compressed_size) < LZ4_COMPRESSBOUND(static_cast<int>(page_size))) {
                returned_size += flushCompressedSave();
            }
